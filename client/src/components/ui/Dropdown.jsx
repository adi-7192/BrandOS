...
export default function Dropdown({ label, tooltip, options = [], error, required, placeholder = 'Select…', className = '', value = '', onChange, ...props }) {
  ...
  return (
    <div className="flex flex-col gap-1">
      ...
      <select
        className={`w-full rounded-lg border border-[var(--brand-border)] bg-[var(--brand-surface)] px-3 py-2.5 text-sm text-[var(--brand-text)] focus:border-[var(--brand-primary)] focus:outline-none focus:ring-2 focus:ring-[var(--brand-primary-soft)] disabled:bg-gray-50 ${error ? 'border-red-400' : ''} ${className}`}
        value={selectValue}
      ...
      {showCustomInput && (
        <input
          ...
          className="mt-2 w-full rounded-lg border border-[var(--brand-border)] bg-[var(--brand-surface)] px-3 py-2.5 text-sm text-[var(--brand-text)] placeholder-[var(--brand-text-muted)] focus:border-[var(--brand-primary)] focus:outline-none focus:ring-2 focus:ring-[var(--brand-primary-soft)]"
        />
      )}
